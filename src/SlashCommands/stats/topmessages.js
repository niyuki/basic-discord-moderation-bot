...
module.exports = new Command({
    name: 'topmessages',
    description: 'See top message ranking in your server',
    userPermissions: [''],
    run: async({client, interaction, args}) => {
        let embed = new MessageEmbed()
        .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL({ dynamic: true })})
        .setFooter(`${interaction.guild.name} guilds message stats`)
    let day = await tm.getDay(interaction.guild.id);
    embed.setDescription(`The message activity of the users on the ${interaction.guild.name} server during the **${day}** day is listed in detail below.`);
