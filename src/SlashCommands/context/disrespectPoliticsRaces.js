...
module.exports = {
    ...
    run: async(client,interaction, args) => {
        ...
        const Database = await warnModel.findOne({ userID: msg.author.id, reason: 'Disrespecting politics'})
        const Database2 = await jailModel.findOne({ userID: msg.author.id, reason: 'Disrespecting politics'})
        ...
        let reason = 'Disrespecting politics'
        ...
        if(!Database) {
            ...
            interaction.guild.members.cache.get(member.user.id).timeout(durationtimeout, 'Disrespecting politics')
            interaction.followUp({ content: member.user.user+ `, looks like you are disrespecting politics. This is your first warning. If you repeat you will get jailed and one more to get banned. (\`${msg.content}\` doesn't fit here so I'm timing him out for ${config.ContextTimeouts.disrespectTimeout})😳`})
            ...
            interaction.followUp({ content: interaction.user+ `, looks like you are disrespecting politics again. Since you were already warned at ${moment(Database.timestamp).format('LLLL')}. (\`${msg.content}\` doesn't fit here so I'll jail him out for ${config.ContextTimeouts.disrespectJail})😳`})
            setTimeout(async() => {
                await warnModel.findOneAndDelete({ userID: member.user.id, reason: 'Disrespecting politics'});
        ...
        await new warnModel({
            ...
            guildID: interaction.guildId,
            moderatorID: interaction.user.id,
            reason: 'Disrespecting politics 3rd Time -> BAN',
            timestamp: Date.now()
        }).save();
        member.ban({ reason});
        await jailModel.findOneAndDelete({ userID: member.user.id, reason: 'Disrespecting politics 2nd Time'});
